...
class UpperCaseEnvironmentConfigurationTest extends AnyFlatSpec with Matchers with MockitoSugar with EnvFixture {
  ...
  it should "search for the original key if upper-case one isn;t found" in {
    setEnv("foo.barBaz42.Qux", "forty-two")
    (new UpperCaseEnvironmentConfiguration).getString("foo.barBaz42.Qux") should equal("forty-two")
  }
